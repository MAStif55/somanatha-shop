...
### F: Destructive Re-Provisioning on Shared VMs (CRITICAL INCIDENT AVOIDANCE)
*   **The Threat:** Previous agents mistakenly assumed they were dealing with a blank slate VM and attempted to push bare-metal bash scripts (`setup-ubuntu-node.sh`) to `apt install` MongoDB, Node, and Nginx. **Executing OS-level provisioning commands on a Multi-Tenant Server will overwrite the existing configurations and completely obliterate all other production databases currently running on the server.**
*   **The Mitigation:** Do NOT attempt to install foundational server software from scratch. MongoDB, Node.js, PM2, and Nginx are already installed and permanently active on the host machine. You must exclusively use local environment variables to connect to new isolated databases (`mongodb://127.0.0.1:27017/levprav_data`) and push local `sites-available` Nginx files for new port mapping. **Never overwrite raw Linux OS daemon settings.**
